...
/******************** Matrix Market internal definitions ********************
   MM_matrix_typecode: 4-character sequence
                                    ojbect 		sparse/   	data
 storage dense     	type        scheme string position:	 [0]        [1]
 [2]         [3] Matrix typecode:  M(atrix)  C(oord)		R(eal)
 G(eneral) A(array)	C(omplex)   H(ermitian) P(attern)   S(ymmetric)
                                                                                I(nteger)	K(kew)
 ***********************************************************************/
...
int mm_read_unsymmetric_sparse(const char *fname, int *M_, int *N_, int *nz_,
    ...
    FILE *f;
    MM_typecode matcode;
    int M, N, nz; // TODO: leave as int for now, so we dont have to change
    ...
    *J_ = J;
    ...
    if (!mm_is_pattern(matcode)) {
        for (i = 0; i < nz; i++) {
            if (fscanf(f, "%d %d %lg\n", &II[i], &J[i],
                       &val[i]) // NOTE: what does the lg affect?
                != 3)
