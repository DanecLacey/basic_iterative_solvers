...
inline void compute_ilu0(const MatrixCRS* A, MatrixCRS* L_strict, MatrixCRS* U_with_diag) {
    int n = A->n_rows;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j_pos = A_ilu.row_ptr[i]; j_pos < A_ilu.row_ptr[i+1]; ++j_pos) {
            row_lookup[A_ilu.col[j_pos]] = j_pos;
        }

        // --- Step 2: Elimination loop ---
        // For each non-zero A(i,k) in the strictly lower part of row i...
        for (int k_pos = A_ilu.row_ptr[i]; k_pos < A_ilu.row_ptr[i+1]; ++k_pos) {
            ...
            if (k < i) {
                ...
                double pivot = 0.0;
                int pivot_idx = -1;
                for (int p_pos = A_ilu.row_ptr[k]; p_pos < A_ilu.row_ptr[k+1]; ++p_pos) {
                ...
                if (std::abs(pivot) < pivot_tolerance) {
                    if (pivot_idx != -1) { // Pivot exists but is small
                        ...
                        A_ilu.val[pivot_idx] = pivot; // Update the matrix with the new pivot
                    } else { // Diagonal entry doesn't exist (structurally singular)
                ...
                for (int j_pos = A_ilu.row_ptr[k]; j_pos < A_ilu.row_ptr[k+1]; ++j_pos) {
                    int j = A_ilu.col[j_pos];
                    if (j > k) { // For each U(k,j)
                        int ij_pos = row_lookup[j]; // Check if A(i,j) exists in pattern
                        if (ij_pos != -1) {
