...
void gs_separate_iteration(Timers *timers, const MatrixCRS *crs_mat_U,
    ...
    TIME(timers->spmv, spmv(crs_mat_U, x, tmp 
        SMAX_ARGS(0, smax, "tmp <- U*x")))

    // tmp <- b - tmp
    TIME(timers->sum, subtract_vectors(tmp, b, tmp, crs_mat_U->n_rows))

    // x <- (D+L)^{-1}(tmp)
    TIME(timers->sptrsv, sptrsv(crs_mat_L, x, D, tmp
        SMAX_ARGS(0, smax, "solve x <- (D+L)^{-1}(b-Ux)")))
}
...
void bgs_separate_iteration(Timers *timers, const MatrixCRS *crs_mat_U,
    ...
    TIME(timers->spmv, spmv(crs_mat_L, x, tmp
        SMAX_ARGS(0, smax, "tmp <- L*x")))

    // tmp <- b - tmp
    TIME(timers->sum, subtract_vectors(tmp, b, tmp, crs_mat_L->n_rows))

    // x <- (D+U)^{-1}(tmp)
    TIME(timers->sptrsv, bsptrsv(crs_mat_U, x, D, tmp 
        SMAX_ARGS(0, smax, "solve x <- (U+L)^{-1}(b-Ux)")))
}
// clang-format on
#endif
