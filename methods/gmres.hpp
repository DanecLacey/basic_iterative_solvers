...
void gmres_separate_iteration(Timers *timers, Solver* solver, int n_solver_iters, double &residual_norm, double *V, double *H, double *H_tmp, double *J, double *Q, double *Q_tmp, double *w, double *R, double *g, double *g_tmp, double *x, double *tmp, double beta, Interface *smax = nullptr) {
    int N = solver->crs_mat->n_cols;
    int local_iter = n_solver_iters - solver->gmres_restart_count * solver->gmres_restart_len;
    TIME(timers->spmv, spmv(solver->crs_mat.get(), &V[local_iter * N], w, local_iter * N, smax, "w_j <- A*v_j"));
    TIME(timers->precond, {
        if (solver->preconditioner == PrecondType::ILU0) {
            apply_preconditioner(solver->L_factor.get(), solver->U_factor.get(), solver->D_factor_vals.get(), w, w, tmp);
        } else {
            apply_preconditioner(solver->preconditioner, N, solver->crs_mat_L_strict.get(), solver->crs_mat_U_strict.get(), solver->D, w, w, tmp, 0, smax, "M^{-1} * w_j");
        }
    });
    orthogonalize_V(timers, N, local_iter, solver->gmres_restart_len, H, V, w);
    least_squares(timers, N, local_iter, solver->gmres_restart_len, J, H, H_tmp, Q, Q_tmp, R);
    update_g(timers, N, local_iter, solver->gmres_restart_len, Q, g, g_tmp, residual_norm, beta);
}
...
#pragma omp parallel for
    ...
    void init_residual() override {
#ifdef USE_SMAX
        compute_residual(crs_mat.get(), x, b, residual, tmp, this->smax, "residual_spmv");
#else
        compute_residual(crs_mat.get(), x, b, residual, tmp);
#endif
        ...
        if (preconditioner == PrecondType::ILU0) {
            apply_preconditioner(L_factor.get(), U_factor.get(), D_factor_vals.get(), residual, residual, tmp);
        } else {
#ifdef USE_SMAX
            apply_preconditioner(preconditioner, crs_mat->n_cols, crs_mat_L_strict.get(), crs_mat_U_strict.get(), D, residual, residual, tmp, 0, this->smax, "init M^{-1} * residual");
#else
            apply_preconditioner(preconditioner, crs_mat->n_cols, crs_mat_L_strict.get(), crs_mat_U_strict.get(), D, residual, residual, tmp);
#endif
        }
        
        double precond_residual_norm = euclidean_vec_norm(residual, crs_mat->n_cols);
        beta = precond_residual_norm;
        g[0] = beta; g_tmp[0] = beta;
...
#ifdef USE_SMAX
        gmres_separate_iteration(timers, this, iter_count, residual_norm, V, H, H_tmp, J, Q, Q_tmp, w, R, g, g_tmp, x, tmp, beta, this->smax);
#else
        gmres_separate_iteration(timers, this, iter_count, residual_norm, V, H, H_tmp, J, Q, Q_tmp, w, R, g, g_tmp, x, tmp, beta);
#endif
    }
