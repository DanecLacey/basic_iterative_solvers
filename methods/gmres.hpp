...
void gmres_separate_iteration(
	...
	*/ 
	...
	for(int j = 0; j <= n_solver_iters; ++j){
		...
		// w_j <- w_j - h_ij*v_k
		subtract_vectors(w, w, &V[j*N], N, H[n_solver_iters + j*restart_len]);
	...
	H[(n_solver_iters + 1)*restart_len + n_solver_iters] = euclidean_vec_norm(w, N); // NOTE: usually 2 norm (Saad)
	...
	scale(&V[(n_solver_iters + 1)*N], w, 1.0/H[(n_solver_iters+1)*restart_len + n_solver_iters], N);
	...
	timers->orthog_time->stop();
	///////////////////// Least Squares Step /////////////////////
	timers->least_sq_time->start();
  ...
  init_dense_identity_matrix(H_tmp, (restart_len + 1), restart_len); 
	...
	if(n_solver_iters == 0){
		...
		dgemm_transpose2(Q, H, H_tmp, (restart_len + 1), (restart_len + 1), restart_len);
	}
	...
	dgemm_transpose2(J, Q, Q_tmp, (restart_len + 1), (restart_len + 1), (restart_len + 1));
	...
	dgemm_transpose2(Q, H, R, (restart_len + 1), (restart_len + 1), restart_len);
