...
void test_gmres_with_ilut_on_good_matrix() {
    ...
    // Create the well-behaved, non-symmetric test matrix.
    auto coo_mat = std::make_unique<MatrixCOO>();
...
DEFINE_SOLVER_TEST(test_gmres_with_ilu0_precond, GMRESSolver, SolverType::GMRES, PrecondType::ILUT)
...
struct RegisterSolverTests {
    RegisterSolverTests() {
        ...
        register_test("Solvers::GMRES_With_ILUT_GoodMatrix", test_gmres_with_ilut_on_good_matrix);
    }
};
