...
void test_native_sptrsv() {
    ...
    std::cout << "        native_sptrsv: 3x3 lower-tri test passed." << std::endl;
...
void test_native_bsptrsv() {
    ...
    std::cout << "        native_bsptrsv: 3x3 upper-tri test passed." << std::endl;
...
void test_vector_operations() {
    ...
    for(int i=0; i<N; ++i) ASSERT_NEAR(result[i], vec1[i] - vec2[i], 1e-9);
    subtract_vectors(result.data(), vec1.data(), vec2.data(), N, 2.0);
    for(int i=0; i<N; ++i) ASSERT_NEAR(result[i], vec1[i] - 2.0*vec2[i], 1e-9);
    ...
    for(int i=0; i<N; ++i) ASSERT_NEAR(result[i], vec1[i] + 3.0*vec2[i], 1e-9);
    ...
    for(int i=0; i<N; ++i) ASSERT_NEAR(result[i], vec1[i] * 5.0, 1e-9);
    std::cout << "        scale test passed." << std::endl;
    copy_vector(result.data(), vec1.data(), N);
    for(int i=0; i<N; ++i) ASSERT_NEAR(result[i], vec1[i], 1e-9);
...
void test_apply_preconditioner() {
    ...
    // Test None
    apply_preconditioner(PrecondType::None, N, nullptr, nullptr, nullptr, vec.data(), rhs.data(), nullptr);
    for(int i=0; i<N; ++i) ASSERT_NEAR(vec[i], rhs[i], 1e-9);
    std::cout << "        PrecondType::None test passed." << std::endl;

    // Test Jacobi
    apply_preconditioner(PrecondType::Jacobi, N, nullptr, nullptr, D.data(), vec.data(), rhs.data(), nullptr);
    for(int i=0; i<N; ++i) ASSERT_NEAR(vec[i], rhs[i] / D[i], 1e-9);
    ...
    // Test GaussSeidel
    {
        ...
        // ... (populate L_strict) ...
        int row_ptr[] = {0, 0, 1, 3}, col[] = {0, 0, 1}; double val[] = {1.0, -2.0, 1.0};
        std::copy(row_ptr, row_ptr + 4, L_strict.row_ptr);
        std::copy(col, col + 3, L_strict.col);
        std::copy(val, val + 3, L_strict.val);
        ...
        apply_preconditioner(PrecondType::GaussSeidel, N, &L_strict, nullptr, D.data(), vec.data(), b_gs.data(), nullptr);
        for(int i=0; i<N; ++i) ASSERT_NEAR(vec[i], expected_x_gs[i], 1e-9);
        std::cout << "        PrecondType::GaussSeidel test passed." << std::endl;
    }
    ...
    // Test BackwardsGaussSeidel
    {
        ...
        // ... (populate U_strict) ...
        int row_ptr[] = {0, 2, 3, 3}, col[] = {1, 2, 2}; double val[] = {1.0, -2.0, 1.0};
        std::copy(row_ptr, row_ptr + 4, U_strict.row_ptr);
        std::copy(col, col + 3, U_strict.col);
        std::copy(val, val + 3, U_strict.val);
        ...
        apply_preconditioner(PrecondType::BackwardsGaussSeidel, N, nullptr, &U_strict, D.data(), vec.data(), b_bgs.data(), nullptr);
        for(int i=0; i<N; ++i) ASSERT_NEAR(vec[i], expected_x_bgs[i], 1e-9);
        std::cout << "        PrecondType::BackwardsGaussSeidel test passed." << std::endl;
    }
...
namespace {
    struct RegisterKernelsTests {
        RegisterKernelsTests() {
            ...
            register_test("Kernels::ApplyPreconditioner", test_apply_preconditioner);
        }
    };
