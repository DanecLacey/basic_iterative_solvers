...
#define ASSERT_EQUAL(val1, val2, name) \
    do { \
        if ((val1) != (val2)) { \
            std::cerr << "Assertion failed: " << (name) << " " << #val1 << " (" << (val1) << ") != " \
                      << #val2 << " (" << (val2) << ")" \
                      << " at " << __FILE__ << ":" << __LINE__ << std::endl; \
            throw std::runtime_error("Assertion failed"); \
        } \
...
void test_extract_L_U() {
    ...
    for(int i=0; i<4; ++i) ASSERT_EQUAL(L.row_ptr[i], exp_L_row_ptr[i], "L.row_ptr");
    ...
    for(int i=0; i<4; ++i) ASSERT_EQUAL(L_strict.row_ptr[i], exp_Ls_row_ptr[i], "L_strict.row_ptr");
    ...
    std::cout << "        extract_L_U: L_strict matrix test passed." << std::endl;
    
    // Expected U (upper + diagonal)
    ASSERT_EQUAL(U.nnz, 6, "U.nnz");
    int exp_U_row_ptr[] = {0, 3, 5, 6};
    for(int i=0; i<4; ++i) ASSERT_EQUAL(U.row_ptr[i], exp_U_row_ptr[i], "U.row_ptr");
    ...
    for(int i=0; i<4; ++i) ASSERT_EQUAL(U_strict.row_ptr[i], exp_Us_row_ptr[i], "U_strict.row_ptr");
    ...
    std::cout << "        extract_L_U: U_strict matrix test passed." << std::endl;
...
void test_peel_diag_crs() {
    ...
    std::cout << "        peel_diag_crs: Diagonal vector extraction passed." << std::endl;
    
    // Check that the matrix was correctly modified (diagonals swapped to end of row data)
    // Expected row 0: [1, 2, 10] with cols [1, 2, 0]
    ...
    std::cout << "        peel_diag_crs: Matrix modification passed." << std::endl;
